...
Public.prototype.getStoreInfo = function(storeSubdomain, callback) {
	retrieve(this.urlBuilder(storeSubdomain, 'store.json'), callback);
}
...
Public.prototype.getProducts = function(storeSubdomain, callback, page, perPage) {
	...
	retrieve(this.urlBuilder(storeSubdomain, path), callback);
}
...
Public.prototype.getProduct = function(storeSubdomain, productId, callback) {
	retrieve(this.urlBuilder(storeSubdomain, 'products/' + productId + '.json'), callback);
}
...
Public.prototype.getCollections = function(storeSubdomain, callback, page, perPage) {
	//pagination not currently working in v1 API
...
Public.prototype.getCollection = function(storeSubdomain, collectionId, callback) {
	retrieve(this.urlBuilder(storeSubdomain, 'collections/' + collectionId + '.json'), callback);
}

// the variant endpoint does not seem to be currently working
// Public.prototype.getVariants = function(storeSubdomain, variantId, callback) {
// 	retrieve(this.urlBuilder(storeSubdomain, 'variants/' + variantId + '.json'), callback);	
// }
